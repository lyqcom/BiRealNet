...
class KaimingNormal(MeInitializer):
    ...
    def _initialize(self, arr):
        tmp = kaiming_normal(arr, self.a, self.mode, self.nonlinearity)
        assignment(arr, tmp)
...
def kaiming_uniform_(weight, a=math.sqrt(5)):
    weight.set_data(weight_init.initializer(KaimingNormal(a=a),
                                            weight.shape,
                                            weight.dtype))


def kaiming_normal_(weight, a=math.sqrt(5)):
    weight.set_data(weight_init.initializer(KaimingUniform(a=a),
                                            weight.shape,
                                            weight.dtype))


def kaiming_normal(arr, a=0, mode='fan_in', nonlinearity='leaky_relu'):
    fan = _calculate_correct_fan(arr, mode)
    gain = calculate_gain(nonlinearity, a)
    std = gain / math.sqrt(fan)
